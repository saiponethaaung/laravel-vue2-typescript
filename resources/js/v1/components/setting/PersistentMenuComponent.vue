    ...
    <div class="inheritHFW ovAuto pageListRootCon">
        ...
        <template v-else>
            <div class="persistentMenuRoot">
                <div class="persistentMenuCon" v-if="selectedSecond==-1">
                    ...
                    <template v-for="(firstMenu, index) in menu">
                        <first-menu-component
                            ...
                            v-on:deleteFirst="deleteFirst(index)"
                            :menu="firstMenu"
                        ></first-menu-component>
                    </template>
                    <template v-if="menu.length<3">
                        <template v-if="adding">
                            <div class="persistentMenuComponent">Loading...</div>
                        </template>
                        <template v-else>
                ...
                <template v-if="selectedFirst!==-1 && menu[selectedFirst].content.type==2 ">
                    <div class="persistentMenuCon">
                        <div class="menuSubHeading">
                            ...
                            <h5
                                class="parentName"
                            >{{ menu[selectedFirst].content.title ? menu[selectedFirst].content.title : '-' }}</h5>
                        </div>
                        <template v-for="(secondMenu, index) in menu[selectedFirst].item">
                            <second-menu-component
                                ...
                                v-on:deleteSecond="deleteSecond(index)"
                                :menu="secondMenu"
                            ></second-menu-component>
                        </template>
                ...
                <template
                    v-if="selectedFirst!==-1 && menu[selectedFirst].content.type==2 && selectedSecond!==-1 && menu[selectedFirst].item[selectedSecond].content.type===2"
                >
                    <div class="persistentMenuCon">
                        <div class="menuSubHeading">
                            ...
                            <h5
                                class="parentName"
                            >{{ menu[selectedFirst].item[selectedSecond].content.title ? menu[selectedFirst].item[selectedSecond].content.title : '-' }}</h5>
                        </div>
                        <template
                            v-for="(thirdMenu, index) in menu[selectedFirst].item[selectedSecond].item"
                        >
                            <third-menu-component
                                :key="index"
                                :index="index"
                                v-on:deleteThird="deleteThird(index)"
                                :menu="thirdMenu"
                            ></third-menu-component>
                        </template>
...
export default class PersistentMenuComponent extends Vue {
        ...
        await Axios({
            ...
            .then(res => {
                for (let i of res.data.data) {
                    this.menu.push(
                        new PersistentMenu(i, this.$store.state.projectInfo.id)
                    );
                }
            })
            .catch(err => {
        ...
        await Axios({
            ...
            .catch(err => {
                if (err.response) {
                    let mesg = this.ajaxHandler.globalHandler(
                        err,
                        "Failed to create new persistent menu!"
                    );
                    alert(mesg);
                }
            });
    ...
    private async createThirdMenu() {
        let createMenu = await this.menu[this.selectedFirst].item[
            this.selectedSecond
        ].createThirdMenu();
            ...
            await Axios({
                ...
                .catch(err => {
                    if (err.response) {
                        let mesg = this.ajaxHandler.globalHandler(
                            err,
                            "Failed to remove persistent menu!"
                        );
                        alert(mesg);
                    }
                });
    ...
    private async deleteThird(index: any) {
        if (confirm("Are you sure you want to delete this menu?")) {
            await Axios({
                ...
                .then(res => {
                    this.menu[this.selectedFirst].item[
                        this.selectedSecond
                    ].item.splice(index, 1);
                })
                .catch(err => {
