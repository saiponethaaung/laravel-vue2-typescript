<template>
    <div class="bcs">
        ...
        <div class="btnClick">
            <router-link
                :to="{'name': 'project.broadcast.sendnow'}"
            >Click here to send your message now</router-link>
        </div>
                    ...
                    <template v-for="(trigger, index) in this.triggerList">
                        <li :key="index" class="broadcastScheduleList">
                            ...
                            >
                                {{ trigger.duration }}
                                <template
                                    v-if="trigger.duration_type===1"
                                >{{ trigger.duration>1 ? 'minutes' : 'minute' }}</template>
                                <template
                                    v-if="trigger.duration_type===2"
                                >{{ trigger.duration>1 ? 'hours' : 'hour' }}</template>
                                <template
                                    v-if="trigger.duration_type===3"
                                >{{ trigger.duration>1 ? 'days' : 'day' }}</template>
                                &nbsp;after&nbsp;
                                <template
                                    v-if="trigger.trigger_type===1"
        ...
        <template v-if="creatingTrigger">
            <div class="btnClick">creating...</div>
        </template>
        <template v-else>
            <div class="btnClick" v-if="triggerLoading">Loading...</div>
        ...
        <template v-else>
            <div class="btnClick" v-if="scheduleLoading">Loading...</div>
...
export default class BroadcastSidebarComponent extends Vue {
    ...
    private updateTrigger() {
        ...
        for (let i in this.triggerList) {
            if (this.triggerList[i].id === this.$store.state.updateTrigger.id) {
                this.triggerList[
                    i
                ].duration = this.$store.state.updateTrigger.duration;
                this.triggerList[
                    i
                ].duration_type = this.$store.state.updateTrigger.duration_type;
                this.triggerList[
                    i
                ].trigger_type = this.$store.state.updateTrigger.trigger_type;
                break;
            }
        }
    }
    ...
    private updateSchedule() {
        ...
        for (let i in this.scheduleList) {
            if (
                this.scheduleList[i].id === this.$store.state.updateSchedule.id
            ) {
                this.scheduleList[i].day = this.$store.state.updateSchedule.day;
                this.scheduleList[
                    i
                ].month = this.$store.state.updateSchedule.month;
                this.scheduleList[
                    i
                ].year = this.$store.state.updateSchedule.year;
                this.scheduleList[
                    i
                ].time = this.$store.state.updateSchedule.time;
                this.scheduleList[
                    i
                ].interval_type = this.$store.state.updateSchedule.interval_type;
                break;
            }
        }
    }
    ...
    private scheduleName(index: number) {
        ...
        switch (this.scheduleList[index].interval_type) {
            case 1:
                name = `${
                    this.$store.state.months[
                        parseInt(this.scheduleList[index].month)
                    ]
                } ${this.scheduleList[index].day} ${
                    this.scheduleList[index].year
            ...
            case 6:
                name = `Every year <span class='float-right'>${
                    this.$store.state.months[
                        parseInt(this.scheduleList[index].month)
                    ]
                } ${this.scheduleList[index].day} • ${time}</span>`;
                break;
