        ...
        <ul class="quickReplyRootContainer" ref="dropdownMenu">
            <li v-for="(qr, index) in content.item" :key="index">
                <div class="quickReplyCapsule">
                    <div class="quickReplyTitle" @click="closeOtherSection(index);qr.canShow=!qr.canShow;">{{ qr.title ? qr.title : 'Enter button name'}}</div>
                    ...
                    <div class="quickReplyInfoBox" :class="{'showInfoBox': qr.canShow}">
                        ...
                        <div>
                            <div>
                                ...
                                <template v-else>
                                    <input v-model="qr.blockKey" @keyup="qr.loadSuggestion()" placeholder="Enter block name"/>
                                    <template v-if="qr.blockList.length>0">
                                        <div class="sugContainer">
                                            <div v-for="(b, index) in qr.blockList" :key="index" class="sugBlock">
                            ...
                            <div>
                                <input placeholder="<Set attribute>" v-model="qr.attribute" v-on:blur="qr.saveContent()">
                            </div>
                            <div>
