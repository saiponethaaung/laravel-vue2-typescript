            ...
            <draggable v-model="blocks" @end="updateBlockOrder" handle="orderBlock">
                ...
                <template v-if="showDelConfirm">
                    <popup-component :type="1">
                        <button
                            class="closePopConfirm"
                            @click="showDelConfirm=false;delBlockIndex=-1;"
                        >
...
export default class SidebarComponent extends Vue {
    ...
    async deleteChatBlock() {
        ...
        if (this.blocks[this.delBlockIndex].canDelete) {
            let deleteBlock = await this.blocks[
                this.delBlockIndex
            ].deleteBlock();
        ...
        for (let i in this.blocks) {
            ...
            for (let s in this.blocks[i].sections) {
                if (
                    ...
                    continue;
                this.blocks[i].sections[
                    s
                ].title = this.$store.state.updateBot.title;
                break;
            }
            break;
        }
    ...
    async updateSectionValid() {
        ...
        for (let i in this.blocks) {
            ...
            for (let s in this.blocks[i].sections) {
                ...
                this.blocks[i].sections[
                    s
                ].check = this.$store.state.updateBotValid.valid;
                break;
            }
            break;
        }
